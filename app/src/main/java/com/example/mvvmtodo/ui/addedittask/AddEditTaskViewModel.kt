...
@ViewModelInject constructor(
    ...
    fun onSaveClick() {
        if (taskName.isBlank()) {
            showInValidINputMssage("Name cannot be empty")
            return
        }

        if (task != null) {
            val updatedTask =
                task.copy(username = task.username, name = taskName, important = taskImportance)
            updatedTask(updatedTask)
        } else {
            val newTask =
                Task(username = username.value, name = taskName, important = taskImportance)
            createTask(newTask)
        }
    }
    ...
    private fun showInValidINputMssage(text: String) = viewModelScope.launch {
